...
# Assuming 'soup' is the new column that combines all relevant attributes
...
df2 = df2.reset_index()
indices = pd.Series(df2.index, index=df2['MovieName']).drop_duplicates()
...
def get_recommendations(title):
    ...
    return_df = pd.DataFrame(columns=['MovieName', 'Homepage', 'ReleaseYear'])
    return_df['MovieName'] = df2['MovieName'].iloc[movie_indices]
    return_df['Homepage'] = df2['MovieName'].iloc[movie_indices]
    return_df['ReleaseYear'] = df2['Year'].iloc[movie_indices]
    return return_df
    ...
    if flask.request.method == 'POST':
        ...
        if m_name not in all_titles:
            ...
            def get_recommendations(title):
                ...
                return return_df
            length=len(df2['original_title'])
            random_number=random.randint(0,length)
            m_name=df2['original_title'][random_number]
